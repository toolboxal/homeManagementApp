...
CREATE TABLE `__new_store_items` (
	...
	`date_status_change` text DEFAULT '2025-03-20' NOT NULL,
...
INSERT INTO `__new_store_items`("id", "name", "date_bought", "date_expiry", "date_status_change", "cost", "status", "expired", "amount", "category", "location_id", "spot_id", "direction_id") SELECT "id", "name", "date_bought", "date_expiry", "date_status_change", "cost", "status", "expired", "amount", "category", "location_id", "spot_id", "direction_id" FROM `store_items`;--> statement-breakpoint
